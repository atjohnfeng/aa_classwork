...
def xnor_select(arr, prc1, prc2)
    ...
    selected = []
    arr.each { |ele| selected << ele if (prc1.call(ele) && prc2.call(ele)) || (!prc1.call(ele) && !prc2.call(ele)) }
    selected
...
def proctition(arr, &prc)
    ...
    procced = []
    arr.each { |ele| procced << ele if prc.call(ele) }
    arr.each { |ele| procced << ele if !prc.call(ele) }
    procced
...
def proctition_platinum(arr, *prc)
    ...
    prc.each_with_index { |x, i| platinum[i+1] = [] }
    arr.each do |ele|
...
def procipher(str, hash)
    ...
    words = str.split(" ").map do |word|
        newword = word
        hash.each do |condition, changer|
            newword = changer.call(word) if condition.call(word)
        end
        newword
    end
